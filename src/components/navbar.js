...
export const Navbar = ({ marca, navegacion, ui, _i18n }) => {
  ...
  return html`
  <nav id="navbar" class="fixed top-0 w-full z-50 transition-all duration-300 ease-apple-soft px-6 py-4">
    <div class="max-w-7xl mx-auto flex justify-between items-center">
      <div class="flex items-center gap-2">
        <div class="w-8 h-8 bg-gradient-to-br from-brand-primary to-brand-secondary rounded-xl"></div>
      ...
      <div class="flex items-center gap-4 md:gap-8">
        <div class="hidden md:flex items-center gap-8 text-sm font-medium text-ink-muted">
          ${navegacion.enlaces.map(
            (enlace) => html`<a href="${enlace.href}" class="hover:text-ink-strong transition-colors duration-300 ease-apple-soft">${enlace.etiqueta}</a>`,
          )}
